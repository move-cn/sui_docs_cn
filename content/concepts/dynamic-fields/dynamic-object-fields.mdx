...
尝试覆盖字段（尝试添加具有已定义的相同 `<Name>` 类型和值的字段）会导致错误，并且事务将失败。您可以通过可变引用对其进行原地修改，也可以通过首先删除旧值来安全地覆盖它们（例如更改其值类型）。

### 访问动态字段

您可以使用以下 API 通过引用引用动态字段：
...
第一个函数直接接受对 `Child` 对象的可变引用，您可以使用尚未添加为 `Parent` 对象字段的 `Child` 对象调用它。
...
类似于解包常规字段中保存的对象，您可以删除动态字段，暴露其值：
