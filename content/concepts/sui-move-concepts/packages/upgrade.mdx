...
例如，[模块初始化程序](../init.mdx)在包升级时不会重新运行。当您发布初始包时，Move 会在发布事件时运行您为包定义的 `init` 函数一次（仅一次）。在随后版本的包中可能包含的任何 `init` 函数都将被忽略。

正如前文所提到的，Sui 网络上的所有包都是不可变的。由于这个事实，您无法从链上删除旧包。因此，没有任何阻止其他包访问已升级包中定义的方法和类型的机制。用户默认情况下也可以选择继续使用包的旧版本。作为包开发者，您必须意识到并考虑到这种可能性。

例如，您可能在原始包中定义一个 `increment` 函数：
...
然后，您的包升级可能会向 `increment` 函数添加一个发射事件：
...
类似于不匹配的函数定义，您可能还会遇到需要与结构体的原始字段保持同步的动态字段维护的问题。为了解决这些问题，您可以在升级的一部分引入新类型，并将用户迁移到新类型上，从而破坏向后兼容性。例如，如果您正在使用拥有对象来演示证明，比如所有权证明，而您开发了新版本的包以解决问题代码，您可以在升级包中引入新类型。然后，您可以向包中添加一个将旧对象交换为新对象的函数。由于您的逻辑仅识别具有新类型的对象，您有效地强制用户进行更新。

另一个让用户更新到最新包的例子是，当您的包中有一个记账的共享对象，您发现它的逻辑存在缺陷，因此无法按预期工作。与前面的例子一样，您希望用户仅使用在具有正确逻辑的升级包中定义的对象，因此您在包升级中添加了新类型和迁移函数。该过程需要进行两个事务，一个用于升级，另一个从升级包中调用，用于设置替换现有对象的新共享对象。为了保护设置函数，您需要创建一个作为包的一部分的 `AdminCap` 对象或类似对象，以确保您作为包的所有者是唯一授权发起该函数的人。甚至更有用的是，您可以在共享对象中包含一个标志，允许您作为包的所有者切换该共享对象的启用状态。您可以添加一个检查已启用状态的条件，以防止在执行迁移时从链上公开访问该对象。当然，您可能仅在预期将来执行此迁移时才会创建此标志，而不是因为您有意开发具有缺陷逻辑的对象。

### 版本化的共享对象

当您创建涉及共享对象的包时，您需要从一开始就考虑升级和版本控制，因为**链上仍然存在包的所有先前版本**。一个有用的模式是为共享对象引入版本控制，并使用版本检查来保护对包中函数的访问。这使您能够将对共享对象的访问限制为包的最新版本。
...
为确保对该包的升级可以将对共享对象的访问限制为包的最新版本，您需要：

1. 在一个常量 `VERSION` 中跟踪模块的当前版本。
2. 在一个新的 `version` 字段中跟踪共享对象 `Counter` 的当前版本。
3. 引入一个 `AdminCap` 以保护特权调用，并通过一个新字段将 `Counter` 与其 `AdminCap` 关联（如果您已经有一个类似的用于共享对象管理的类型，那么可以重用该类型）。该 cap 用于保护从版本到版本迁移共享对象的调用。
...
这种模式构成了涉及共享对象的可升级包的基础，但您可以根据包的需求以多种方式进行扩展：

- 版本约束可以更具表现力：
  - 而不是使用单个 `u64`，版本可以被指定为一个 `String`，或者一个上下界的对。
  - 您可以通过将标记类型添加到共享对象的动态字段上来控制对特定函数或函数集的访问。
- `migrate` 函数可以变得更复杂（修改共享对象中的其他字段，添加/删除动态字段，同时迁移多个共享对象）。
- 您可以实现需要在多个事务中运行的大型迁移的三阶段设置：
...
要升级包，您的包必须满足以下要求：
- 您必须拥有要升级的包的 `UpgradeTicket`。当您发布包时，Sui 网络会发出 `UpgradeCap`，然后您可以作为该 `UpgradeCap` 的所有者发出 `UpgradeTicket`。Sui 客户端 CLI 会自动处理此要求。
- 您的更改必须与先前版本的布局兼容。
    - 现有的 `public` 函数签名和结构布局必须保持不变。
    - 您可以添加新的结构和函数。
    - 您可以从现有函数（公共或其他）中删除通用类型约束。
    - 您可以更改函数实现。
    - 您可以更改非 `public` 函数签名，包括 `friend` 和 `entry` 函数签名。

:::info

如果您的包有一个依赖项，并且该依赖项已升级，则您的包不会自动依赖于新版本。您必须显式将自己的包升级到新的依赖项。
...
- `--cap`：`UpgradeCap` 的 ID。您可以在发布命令的返回中获得此 ID。
...
结果包括一个**对象更改**部分，其中包含升级所需的两个信息，一个 `UpgradeCap` ID 和您的包 ID。

您可以使用响应中的 `Object.objectType` 值来识别不同的对象。`UpgradeCap` 条目的值为 `String("0x2::package::UpgradeCap")`，包的 `objectType` 为 `String("<PACKAGE-ID>::sui_package::<MODULE-NAME>")`。

为确保您的其他包可以将此包用作依赖项，您必须更新包的清单（Move.toml 文件）以包含此信息。
...
过了一段时间，您决定升级您的 `sui_package` 以包含一些请求的功能。在运行 `upgrade` 命令之前，您需要再次编辑清单。在 `[addresses]` 部分中，将 `sui_package` 地址值再次更新为 `0x0`，以便验证器为升级包发行一个新地址。您可以保持 `published-at` 值不变，因为在发布依赖包时，此值仅由工具链读取。保存的清单现在类似于以下内容：
...
具有新清单和代码后，现在是使用 `sui client upgrade` 命令升级您的包的时候了。将 `UpgradeCap` ID（示例中的 `<UPGRADE-CAP-ID>` 值）传递给 `--upgrade-capability` 标志：

```shell
sui client upgrade --gas-budget <GAS-BUDGET-AMOUNT> --upgrade-capability <UPGRADE-CAP-ID>
```

如果新包不符合 [requirements](#requirements)，控制台会向您发出警告，否则编译器会将升级后的包发布到网络并返回结果：
...
结果提供了升级包的新 ID。与升级之前一样，您需要在清单中包含该信息，以便依赖于您的 `sui_package` 的任何其他包都知道在哪里找到链上的字节码进行验证。再次编辑清单，为 `published-at` 值提供升级包的 ID，并在 `[addresses]` 部分中返回原始的 `sui_package` ID 值：
...
`published-at` 值在每次升级时都会更改。在 `[addresses]` 部分中，`sui_package` 的 ID 始终指向升级后的原始包 ID。然而，在运行 `upgrade` 命令之前，您必须始终将该值更改回 `0x0`，以便验证器知道为升级创建一个新的 ID。
