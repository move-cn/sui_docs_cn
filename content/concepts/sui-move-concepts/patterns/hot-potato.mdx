---
title: 热土豆 (Hot Potato)
---

import ExampleImport from '@site/src/components/ExampleImport';

热土豆是一个没有能力的结构，因此您只能在其模块中打包和解包它。在这个结构中，在函数 A 返回一个热土豆并且函数 B 消耗它的情况下，您必须在函数 A 之后调用函数 B。例如，在实现闪电贷款时，热土豆模式非常有用。
...
Sui存储库包含多个示例，演示了Sui的各种功能。这些示例中的[闪电贷款](https://github.com/MystenLabs/sui/blob/main/examples/move/flash_lender/sources/example.move)代码演示了本主题描述的模式。
