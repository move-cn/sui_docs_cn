...
能力是一种允许使用对象进行授权操作的模式。例如，[`UpgradeCap`](../packages/upgrade.mdx#requirements) 授权了升级包的操作。您可能会遇到的最常见的能力之一是 `TreasuryCap`，它在 [`sui::coin`](https://github.com/MystenLabs/sui/tree/main/crates/sui-framework/packages/sui-framework/sources/coin.move) 中定义。
