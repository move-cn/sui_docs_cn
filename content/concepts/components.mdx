...
Sui 这个名称在英语中发音为 `swē`，源自日语中表示水元素的一个词汇。您可以在区块链的名称、其本地代币元素 (SUI和MIST) ，以及其品牌形象中看到这种影响。
...
一切都有成本，区块链交易也不例外。提供计算能力来处理交易块并存储其结果是需要花费的。处理交易块的成本术语是“gas”。您使用区块链的本地代币，这种情况下是SUI（或MIST），支付gas和存储数据的成本。
...
- **Mainnet：** 处理生产交易块的网络。当您交易SUI或最终基于法定货币的NFT时，您是在Sui的Mainnet网络上进行的。
...
- **Localnet：** 您可以在本地计算机上运行Sui网络。在受控制的环境中，使用本地网络进行开发提供了一种优化的工作流程。
