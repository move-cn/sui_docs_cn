...
本指南将向您展示如何编写并部署一个使用 reCAPTCHA 验证用户在与合同交互之前是否为人类（而不是机器人）的 Move 智能合约。CAPTCHA 是一种机器人缓解方法，要求您通过挑战测试来证明您是人类。CAPTCHA 测试在阻止机器人执行任务方面是有效的，但如果测试过于困难或频繁，对于合法用户来说可能变得令人讨厌或令人沮丧。reCAPTCHA 是 CAPTCHA 测试的一种形式。

本指南假定您已经 [安装了 Sui](../getting-started/sui-install.mdx) 并了解 Sui 的基础知识。

## Move 智能合约

与所有 Sui dApp 一样，链上的一个 Move 包驱动着 reCAPTCHA 模块的逻辑。以下说明将引导您创建和发布该模块。

### reCAPTCHA 模块

在开始之前，您必须初始化一个 Move 包。在您要存储示例的目录中打开终端或控制台，并运行以下命令以创建一个名为 `recaptcha` 的空包：
...
现在您已经实现了 `verify`，可以继续下一步，演示如何与合约交互。编写一个 `interact` 函数，检查用户是否已验证。
...
要实现 reCAPTCHA 的后端，您需要创建一个可以处理 HTTP 请求和响应的 `express` 应用程序。您还需要安装一些依赖项，如 `@noble/ed25519`、`axios`、`cors`、`helmet`、`morgan` 和 `dotenv`。这些包可帮助您处理密码学、HTTP 请求、跨域资源共享、安全性、日志记录和环境变量。
...
要实现 reCAPTCHA 的前端，您需要创建一个 React 应用程序，它可以渲染用户界面并与后端和智能合约交互。您还需要安装一些依赖项，如 `@mysten/dapp-kit`、`@mysten/sui.js`、`axios` 和 `react-google-recaptcha`。这些包可帮助您集成钱包、执行事务、处理 HTTP 请求以及呈现 reCAPTCHA。
