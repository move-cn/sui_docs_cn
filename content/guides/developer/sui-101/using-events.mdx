...
为了支持活动监控，Move 在 Sui 网络上提供了一个发出事件的结构。当您与 Sui 网络建立连接时，实际上是在您的客户端和 Sui 网络节点之间创建了一个双向的交互式通信会话。有了这个打开的会话，您可以订阅 Sui 网络添加到事件流中的特定事件，以实现对事件的实时监控。
...
如果要订阅链上的事件，首先需要知道有哪些事件可用。通常您可能已经知道或者可以发现您自己的代码发出的事件，但当您需要订阅您不拥有的包的链上事件时，情况就变得不那么直观了。Sui RPC 提供了一个 [queryEvents](/sui-api-ref#suix_queryEvents) 方法，用于查询链上的包并返回可以订阅的可用事件。

## 过滤事件

您可以过滤您的代码用于查询或订阅的事件。这两种过滤选项相似，但也有一些差异。
...
在添加了依赖之后，您可以使用 emit 函数在您想要监视的动作发生时触发事件。例如，以下代码是一个使用数字甜甜圈的示例应用程序的一部分。collect_profits 函数负责收集 SUI，并在每次调用该函数时触发一个事件。要对该事件采取行动，您需要订阅它。
...
在真空中触发事件并不是非常有用。您还需要有能力监听这些事件，以便可以对其进行操作。在 Sui 中，您订阅这些事件并提供在事件触发时触发的逻辑。

Sui 完整节点支持使用通过 WebSocket API 传输的 JSON-RPC 通知进行订阅功能。您可以直接与 RPC 进行交互 [suix_subscribeEvent](/sui-api-ref#suix_subscribeEvent), [suix_subscribeTransaction](/sui-api-ref#suix_subscribeTransaction)，或者您可以使用像 Sui TypeScript SDK 这样的 SDK。以下是来自其中一个示例的摘录，该示例使用 TypeScript SDK 对在筛选器中标识的内容创建了一个异步订阅。
...
Move 智能合约可以调用其他发出事件的智能合约。例如，Deepbook_utils 可以调用 dee9 智能合约并发出此事件。请注意，使用包、事务模块来查询 dee9/clob_v2 会漏掉以下事件，即使这实际上是 dee9 包发出的事件。当前对这个问题的解决方法是了解您关心的所有 packageId，并在 queryEvent 调用中搜索这些内容。
...
要创建事件订阅，您可以使用一个基本的 Node.js 应用程序。您需要 Sui TypeScript SDK，因此在项目的根目录中使用 `npm install @mysten/sui.js` 安装模块。在您的 TypeScript 代码中，从库中导入 `JsonRpcProvider` 和一个连接。
