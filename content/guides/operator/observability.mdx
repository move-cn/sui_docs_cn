...
在跨度中定义的键值对属性，包括 `tx_digest` 和添加的任何其他字段，每个发生在跨度内的日志消息都会继承。日志消息可以设置它们自己的键和值。日志继承跨度属性的事实允许您跟踪例如事务跨越线程和进程边界的流程。
...
例如，您可以通过本地辅助日志转发器（例如 [Vector](https://vector.dev)）将 JSON 日志馈送到 JSON 日志，并继续发送到 ElasticSearch 等目的地。
...
要使内存分析正常工作，您需要设置环境变量 `_RJEM_MALLOC_CONF=prof:true`。如果使用 [Docker 镜像](https://hub.docker.com/r/mysten/sui-node)，它们会自动设置。
