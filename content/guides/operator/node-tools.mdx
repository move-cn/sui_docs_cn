...
1. 确保您已完成所有[先决条件](../developer/getting-started/sui-install.mdx#prerequisites)。

2. 构建 `sui` 二进制文件，您需要用于创世典礼。此步骤可以在任何您喜欢的机器上完成。它不必在运行验证器的机器上执行。
...
3. 运行以下命令设置您的 Sui 帐户和 CLI 环境。

    1. 如果这是第一次运行此程序，它将要求您提供 Sui Fullnode Server URL 和有意义的环境别名。它还将在 `sui.keystore` 中生成一个随机的密钥对和一个配置文件 `client.yaml`。如果您已经拥有验证器帐户密钥，请将其替换。

    2. 如果您已经设置好了，请确保
      a. `client.yaml` 中的 `rpc` 是正确的。
      b. `client.yaml` 中的 `active_address` 是正确的。
      b. `sui.keystore` 包含您的帐户密钥对。
...
```mdx
4. 为了测试您已连接到网络并正确配置了配置，请运行以下命令显示您的验证器信息。
...
如果从第 5 到第 11 点更改任何元数据，它们将仅在下一个时期之后更改 - 对于这些元数据，您将希望在下一个时期后立即重新启动验证器程序，使用新的密钥文件和/或更新的 `validator.yaml` 配置。特别确保新地址没有被防火墙阻挡。
...
这将生成一个 `validator.info` 文件和密钥对文件。此命令的输出包括：
  1. 四个密钥对文件（[更多信息](./validator-tasks.mdx#key-management)）。 ==使用最小可见性设置它们的权限（例如 chmod 600）并安全地存储它们==。在运行以下验证器节点时，它们是必需的，如下面所述。
    a. 如果您仔细遵循本指南，此密钥对实际上是从您的 `sui.keystore` 文件复制的。
...
在此时，您是验证器候选人，可以开始接受自我质押和委托质押。

**如果您还没有，请现在启动一个全节点以赶上网络。当您正式加入委员会但未完全更新时，您无法对网络进行有意义的贡献，并且可能会受到对等体报告的影响，因此面临您和您的委托人质押奖励降低的风险。**
